...
def shortest_path(node_list, edges, start):
    ...
    return visited, round(total_weight,3) 
...
def find_path(orders):
    print("*************************orders*******Dijikstra")
    print(orders)
    ...
    media_dir = "F:\\Walket-Django\\Walket\\media\\" 
    imgnames = []
    batchnos= []
    shortest_paths = []
    costs = []
    ...
    for i in orders:
        ...
        print("------------------------------------------------------------------------")
        batchnos.append(cnt)
        shortest_paths.append(shortest_path_taken)
        costs.append(weight)
        actual_costs.append(actual_weight)
       
        im = array(Image.open(media_dir+"Map.png"))
        ...
        x=[]
        y=[]
        colors=colors=['#660033', '#ffff00','#003300', '#9467bd','#800040',
        ...
        if fs.exists(str(cnt)+'.png'): fs.delete(str(cnt)+'.png')
        imgnames.append("path"+str(cnt)+'.png')
        plt.savefig(media_dir+'path'+str(cnt)+'.png', dpi=300, bbox_inches='tight') 
        plt.close()
        cnt+=1 
    ...
    return batchnos, shortest_paths, costs,actual_costs, imgnames
